    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if step % args.disp_interval == 0:
                ...
                print("\t\t\tdloss s1: %.4f, dloss t1: %.4f, dloss s2: %.4f, dloss t2: %.4f" % (dloss_s1, dloss_t1, dloss_s2, dloss_t2))
                print("\t\t\tdloss s3: %.4f, dloss t3: %.4f, dinst_s: %.4f, dinst_t: %.4f" % (dloss_s3, dloss_t3, dloss_s_p, dloss_t_p))
                print("\t\t\teta: %.4f" % (args.eta))
        ...
        save_name = os.path.join(output_dir,
                                 'SCL_{}_eta_{}_gamma_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t, args.eta, args.gamma, args.session, epoch, step))
        save_checkpoint({
