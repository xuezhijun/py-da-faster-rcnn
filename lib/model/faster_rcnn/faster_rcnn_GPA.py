...
from model.rpn.rpn_GPA import _RPN  ####
...
from model.utils.net_utils import get_adj  ####
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes,
        ...
        if self.training:
            ...
            rois, rpn_loss_cls, rpn_loss_bbox, rpn_cls_prob = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes)  # return 4 args
            tgt_rois, tgt_rpn_loss_cls, tgt_rpn_loss_bbox, tgt_rpn_cls_prob = self.RCNN_rpn(tgt_base_feat, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)
            ...
            # feed base feature map to RPN to obtain rois
            rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes)  # return 3 args
        ...
        if self.training:
            rpn_adapt_feat = self.rpn_adapt_feat(pooled_feat.view(pooled_feat.size(0), -1))  # ROI Pooling, Linear: 7*7*512-->128
            tgt_rpn_adapt_feat = self.rpn_adapt_feat(tgt_pooled_feat.view(tgt_pooled_feat.size(0), -1))

        # feed pooled features to top model
        pooled_feat = self._head_to_tail(pooled_feat)  # after vgg.classifier, 4096
        ...
        adapt_feat = self.RCNN_adapt_feat(pooled_feat)  # Linear: 4096-->64
        tgt_adapt_feat = self.RCNN_adapt_feat(tgt_pooled_feat)

        cls_score = self.RCNN_cls_score(pooled_feat)  # Linear: 4096->n_classes
        ...
        if self.training:
            ...
            if self.mode == 'adapt':
                RCNN_loss_intra, RCNN_loss_inter = self.adaptive_loss(adapt_feat, cls_prob, tgt_adapt_feat, tgt_cls_prob, batch_size)
            # use gcn to cluster the representation of every class
            elif self.mode == 'gcn_adapt':
                RCNN_loss_intra, RCNN_loss_inter = self.gcn_adaptive_loss(adapt_feat, cls_prob, rois, tgt_adapt_feat, tgt_cls_prob, tgt_rois, batch_size)

            # intra-class and inter-class losses for RPN
            # pull same classes and push away different classes of source and target domains
            if self.rpn_mode == 'adapt':
                RPN_loss_intra, RPN_loss_inter = self.adaptive_loss(rpn_adapt_feat, rpn_cls_prob, tgt_rpn_adapt_feat, tgt_rpn_cls_prob, batch_size)
            # use gcn to cluster the representation of every class
            elif self.rpn_mode == 'gcn_adapt':
                RPN_loss_intra, RPN_loss_inter = self.gcn_adaptive_loss(rpn_adapt_feat, rpn_cls_prob, rois, tgt_rpn_adapt_feat, tgt_rpn_cls_prob, tgt_rois, batch_size)
            ########
        ...
        if self.training:
            return rois, tgt_rois, cls_prob, tgt_cls_prob, bbox_pred, tgt_bbox_pred, \
                   rpn_loss_cls.view(-1), tgt_rpn_loss_cls.view(-1), rpn_loss_bbox.view(-1), tgt_rpn_loss_bbox.view(-1), \
                   RCNN_loss_cls.view(-1), tgt_RCNN_loss_cls.view(-1), RCNN_loss_bbox.view(-1), tgt_RCNN_loss_bbox.view(-1), \
                   RCNN_loss_intra.view(-1), RCNN_loss_inter.view(-1), \
                   rois_label, tgt_rois_label, \
                   RPN_loss_intra.view(-1), RPN_loss_inter.view(-1)
        else:
            return rois, tgt_rois, cls_prob, tgt_cls_prob, bbox_pred, tgt_bbox_pred, \
                   rpn_loss_cls, tgt_rpn_loss_cls, rpn_loss_bbox, tgt_rpn_loss_bbox, \
                   RCNN_loss_cls, tgt_RCNN_loss_cls, RCNN_loss_bbox, tgt_RCNN_loss_bbox, \
                   RCNN_loss_intra, RCNN_loss_inter, \
                   rois_label, tgt_rois_label, \
                   RPN_loss_intra, RPN_loss_inter
    ...
    def adaptive_loss(self, pooled_feat, cls_prob, tgt_pooled_feat, tgt_cls_prob, batch_size, epsilon=1e-6):
        ...
        tgt_pooled_feat = tgt_pooled_feat.view(batch_size, tgt_pooled_feat.size(0) // batch_size, tgt_pooled_feat.size(1))
        ...
        for i in range(class_feat.size(0)):
            ...
            for j in range(i + 1, class_feat.size(0)):
                ...
                inter_loss = inter_loss + torch.pow(
                    (self.margin - torch.sqrt(self.distance(tmp_src_feat_1, tmp_src_feat_2))) / self.margin, 2) * torch.pow(
                    torch.max(self.margin - torch.sqrt(self.distance(tmp_src_feat_1, tmp_src_feat_2)), torch.tensor(0).float().cuda()), 2.0)

                inter_loss = inter_loss + torch.pow(
                    (self.margin - torch.sqrt(self.distance(tmp_tgt_feat_1, tmp_tgt_feat_2))) / self.margin, 2) * torch.pow(
                    torch.max(self.margin - torch.sqrt(self.distance(tmp_tgt_feat_1, tmp_tgt_feat_2)), torch.tensor(0).float().cuda()), 2.0)

                inter_loss = inter_loss + torch.pow(
                    (self.margin - torch.sqrt(self.distance(tmp_src_feat_1, tmp_tgt_feat_2))) / self.margin, 2) * torch.pow(
                    torch.max(self.margin - torch.sqrt(self.distance(tmp_src_feat_1, tmp_tgt_feat_2)), torch.tensor(0).float().cuda()), 2.0)

                inter_loss = inter_loss + torch.pow(
                    (self.margin - torch.sqrt(self.distance(tmp_tgt_feat_1, tmp_src_feat_2))) / self.margin, 2) * torch.pow(
                    torch.max(self.margin - torch.sqrt(self.distance(tmp_tgt_feat_1, tmp_src_feat_2)), torch.tensor(0).float().cuda()), 2.0)
